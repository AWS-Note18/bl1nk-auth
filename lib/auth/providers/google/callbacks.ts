...
// ────────────────────────────────────────────────
export class GoogleCallbackHandler implements CallbackHandler {
  async handle(code: string, state: OAuthState): Promise<CallbackResult> {
    try {
      ...
      const accessToken = await googleStrategy.exchangeCode(code);

      // Fetch user profile
      const profile = await googleStrategy.fetchProfile(accessToken);
...
// ────────────────────────────────────────────────
export const googleCallbackHandler = new GoogleCallbackHandler();
